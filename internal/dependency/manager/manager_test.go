...
func (s *managerTestingSuite) TestGetUpdateFor() {
	...
	if s.CheckForUpdatesShouldFailWith != nil {
		assert.EqualError(s.T(), err, s.CheckForUpdatesShouldFailWith.Error())
	} else {
...
func (s *managerTestingSuite) TestManagerInstallDependency() {
	...
	if s.CommandInstallShouldFailWith != nil {
		assert.EqualError(s.T(), err, s.CommandInstallShouldFailWith.Error())
	} else {
		assert.NoError(s.T(), err)
...
func TestInstallMajorUpdateWithoutDefaultRegistryFallback(t *testing.T) {
	...
	mts := managerTestingSuite{
		...
		CheckForUpdatesShouldFailWith: &fs.PathError{Op: "open", Err: fmt.Errorf("The system cannot find the file specified."),
		},
...
func TestInstallMajorUpdateWithNonExistentHttpRegistry(t *testing.T) {
	...
	mts := managerTestingSuite{
		...
		CheckForUpdatesShouldFailWith: &fs.PathError{Op: "open", Path: "http://fake.registry.io", Err: fmt.Errorf("The filename, directory name, or volume label syntax is incorrect.")},
